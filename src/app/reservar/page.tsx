        ...
        return (
          <div key={i} className="flex items-center">
            <div className="flex flex-col items-center gap-1">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${done ? 'bg-sage-500 text-white' : active ? 'bg-sage-500 text-white ring-4 ring-sage-100' : 'bg-sage-100 text-sage-400'}`}>
      ...
      <main className="container py-10 max-w-3xl mx-auto">
        ...
        {step === 0 && (
          <div className="bg-white border border-sage-200 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-6">
              <button onClick={() => { if (viewMonth === 0) { setViewMonth(11); setViewYear(y=>y-1) } else setViewMonth(m=>m-1) }} className="p-2 rounded-lg hover:bg-sage-50 text-sage-500">
                <ChevronLeft size={20} />
              </button>
              <h2 className="font-serif text-xl text-sage-900">{MONTH_NAMES[viewMonth]} {viewYear}</h2>
              <button onClick={() => { if (viewMonth === 11) { setViewMonth(0); setViewYear(y=>y+1) } else setViewMonth(m=>m+1) }} className="p-2 rounded-lg hover:bg-sage-50 text-sage-500">
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <button key={day} disabled={past || !avail} onClick={() => setSelectedDate(date)}
                    className={`aspect-square rounded-xl text-sm font-medium transition-all flex items-center justify-center relative
                      ${sel ? 'bg-sage-500 text-white' : avail ? 'hover:bg-sage-100 text-sage-900' : 'text-sage-300 cursor-not-allowed'}`}>
                    {day}
                    {avail && !sel && <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-sage-400" />}
                  </button>
                )
              })}
            </div>
            <div className="mt-6 flex justify-end">
              <button disabled={!selectedDate} onClick={() => setStep(1)} className="btn-primary disabled:opacity-40">Continuar →</button>
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="bg-white border border-sage-200 rounded-2xl p-6">
            ...
            <p className="text-sm text-sage-400 mb-6">{selectedDate?.toLocaleDateString('es-CL', { weekday:'long', day:'numeric', month:'long' })}</p>
              ...
              <div className="grid grid-cols-4 sm:grid-cols-6 gap-3">
                {availableHours.map(h => (
                  <button key={h} onClick={() => setSelectedHour(h)}
                    className={`py-3 rounded-xl text-sm font-medium border transition-all ${selectedHour === h ? 'bg-sage-500 text-white border-sage-500' : 'border-sage-200 text-sage-700 hover:border-sage-400 hover:bg-sage-50'}`}>
                    {h}
                  </button>
                ))}
              </div>
            ...
            <div className="mt-6 flex justify-between">
              ...
              <button disabled={!selectedHour} onClick={() => setStep(2)} className="btn-primary disabled:opacity-40">Continuar →</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="bg-white border border-sage-200 rounded-2xl p-6">
              ...
              {availablePsychs.map(p => {
                ...
                return (
                  <button key={p.id} onClick={() => setSelectedPsych(p)}
                    onMouseEnter={() => setHoveredPsych(p.id)} onMouseLeave={() => setHoveredPsych(null)}
                    className={`rounded-2xl p-4 text-center border transition-all ${sel ? 'border-sage-500 bg-sage-50 ring-2 ring-sage-400 ring-offset-1' : 'border-sage-200 hover:border-sage-400 hover:bg-sage-50'}`}>
                    <div className="w-16 h-16 rounded-full mx-auto mb-3 flex items-center justify-center text-white font-serif text-2xl font-semibold border-2 border-white shadow-sm" style={{ backgroundColor: color }}>
                      {initials}
                    </div>
            ...
            <div className="mt-6 flex justify-between">
              ...
              <button disabled={!selectedPsych} onClick={() => setStep(3)} className="btn-primary disabled:opacity-40">Continuar →</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="bg-white border border-sage-200 rounded-2xl p-6">
            ...
            <div className="mt-6 flex justify-between">
              <button onClick={() => setStep(2)} className="btn-ghost border border-sage-200 text-sm px-5">← Volver</button>
