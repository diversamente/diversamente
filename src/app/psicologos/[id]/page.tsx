...
import { getAvatarColor, getInitials, DAY_LABELS, HORARIO_NORMAL, HORARIO_PRIME } from '@/lib/data'
      ...
      <main className="container py-8">
        ...
        <div className="bg-sage-50 border border-sage-200 rounded-3xl p-8 flex flex-col md:flex-row gap-6 items-start mb-8">
          ...
          <div className="flex-1">
            ...
            <p className="text-sage-500 text-sm font-medium mb-3">
              Psicólog{psych.nombre.endsWith('a') ? 'a' : 'o'} Clínic{psych.nombre.endsWith('a') ? 'a' : 'o'}
            </p>
          ...
          <div className="md:text-right">
            ...
            <Link href="/tarifas" className="btn-primary text-sm">
              Reservar Sesión
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {/* Schedule */}
            <div className="bg-white border border-sage-200 rounded-2xl p-6">
              <h2 className="font-serif text-xl text-sage-900 mb-4 pb-3 border-b border-sage-100">Disponibilidad</h2>
              <p className="text-xs font-medium text-sage-500 uppercase tracking-wide mb-3">Horario Normal (08:00–20:00)</p>
              <div className="overflow-x-auto">
                <table className="w-full text-xs border-collapse">
                  <thead>
                    <tr>
                      <th className="text-sage-400 text-right pr-3 pb-1 w-16">Hora</th>
                      {Object.keys(DAY_LABELS).map(d => (
                        <th key={d} className="bg-sage-50 rounded px-1 pb-1 text-sage-500 font-medium text-center">
                          {DAY_LABELS[d]}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {HORARIO_NORMAL.map(h => (
                      <tr key={h}>
                        <td className="text-sage-400 text-right pr-3 py-0.5">{h}</td>
                        {Object.keys(DAY_LABELS).map(d => {
                          const avail = (psych.horarioNormal[d as keyof typeof psych.horarioNormal] || []).includes(h)
                          return (
                            <td key={d} className="p-0.5">
                              <div className={`h-5 rounded text-center ${avail ? 'bg-sage-500' : 'bg-sage-50 border border-sage-100'}`} />
                            </td>
                          )
                        })}
                      </tr>
          ...
          <div className="space-y-6">
            <div className="bg-white border border-sage-200 rounded-2xl p-5">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex items-center gap-2 text-sage-500">
                  <GraduationCap size={15} className="text-sage-400 flex-shrink-0" />
                  {psych.universidad}
                </div>
                <div className="flex items-center gap-2 text-sage-500">
                  <MapPin size={15} className="text-sage-400 flex-shrink-0" />
                  Online (videollamada)
                </div>
                <div className="flex items-center gap-2 text-sage-500">
                  <Globe size={15} className="text-sage-400 flex-shrink-0" />
                  Español
                </div>
                <div className="flex items-center gap-2 text-sage-500">
                  <CheckCircle size={15} className="text-sage-400 flex-shrink-0" />
                  Acepta Isapres
                </div>
                <div className="flex items-center gap-2 text-sage-500">
                  <Clock size={15} className="text-sage-400 flex-shrink-0" />
                  Sesiones de 50 min
                </div>
              </div>
            </div>
            ...
            <div className="bg-sage-500 rounded-2xl p-5 text-white text-center">
              ...
              <Link
                href="/tarifas"
                className="bg-white text-sage-600 hover:bg-sage-50 text-sm font-medium
                           px-5 py-2 rounded-full transition-colors block"
              >
                Reservar Sesión
              </Link>
              <p className="text-sage-200 text-xs mt-3">Sin pago por adelantado</p>
            </div>
          </div>
        </div>
      </main>
